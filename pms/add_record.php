...
if(isset($_GET['d_id'])){
    $DoctorID = $_GET['d_id'];
    $getDoctorInfo = mysql_query("SELECT * FROM ".$prev."patient WHERE `p_id` = '$DoctorID'");
    $doctorResultShow = mysql_fetch_array($getDoctorInfo);
    if($doctorResultShow['gender'] == "Male"){
        $stat = "Dr.";
    }else{
        $stat = "Dra.";
    }
...
<!------ Script And Style ----->
 ...
 <h4 style="margin-left:22px;">Add Record Process</h4>
